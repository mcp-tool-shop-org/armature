...
| État | **Version 0.4.0 sur les registres ; cet arbre est dépassé.** L’exécution de conformité (832 résultats) est toujours l’état publié. Depuis, il y a eu l’étape D, un peaufinage visuel, puis une étape qui a ajouté un soumetteur autorisé dans le dépôt pour la transaction (`build_submit_payload.py`, exécution à blanc, pas de crédits actifs dans la suite), qui a augmenté la CLI installée (`canon` / `verify` / `spec` / `donor`) et qui a enregistré la répétition **34098347849**. Rien de nouveau n’est étiqueté tant que vous n’en avez pas supprimé un. |

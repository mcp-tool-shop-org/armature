...
| Points de contrôle | **La porte CANON** refuse une soumission payante dont le sujet ne peut pas être associé à un canon lisible par machine. La surface est la ligne, un occupant nul est un **trou plutôt qu’une absence**, et les deux directions sont vérifiées (l’invite couvre le canon ; tout ce qui se trouve dans l’invite *est* canon). Elle se déclenche **avant** la création du répertoire de sortie, dans chacun des sept générateurs de charge utile, car l’étape irréversible dont ce dépôt est responsable est l’écriture d’une charge utile. La solution est basée sur un recensement : `--no-canon` pour un sujet qui *possède* un canon est refusé, et non accepté — et, depuis le premier contrôle de conformité, elle est **très claire pour chaque transaction** : chaque générateur affiche `[canon] ARMED: <subject>`, et lors de la solution de recensement `[canon] UNGATED: <subject> — <the census row's reason>`, de sorte qu’un journal de construction distingue un trou validé d’un sujet dont le canon n’a jamais été écrit ; et enregistre le verdict sous `gates.CANON`, de sorte qu’aucun enregistrement ne puisse laisser la question de savoir si le canon a été activé ou désactivé sans réponse. |
| Tests | **7538 réussis sur le banc d’essai** (64 sauts, mesurés le 2026-09-06 à la fin de l’exécution de conformité — 1359 avant, 1781 après la première passe, 7181 après l’étape B), identiques sous `-O` ; les tests CI simulent ce qu’un exécuteur peut honnêtement faire — les actifs locaux du banc d’essai **sont clairement ignorés**. |
| État | **v0.4.0** — l’exécution de conformité : 832 problèmes corrigés en trois étapes, un contrat d’arrêt pour chaque instrument, chaque refus indiquant ce qu’il a mesuré, et une suite cinq fois plus grande. La v0.3.0 a donné au journal un point de contrôle et un index qui se vérifie lui-même. `armature_core` est envoyé à PyPI sous la forme `armature-studio` et à npm sous la forme `@mcptoolshop/armature-studio`, publié à partir d’une étiquette par OIDC sans jeton à longue durée de vie. |
...
`armature_core` s’installe à partir de PyPI (ci-dessus) ; le **journal d’expériences et les instruments de rendu** sont ce dépôt, cloné et exécuté — aucun service, aucun démon. Chaque instrument est appelé directement :
...
Chaque instrument CPython se termine avec le code **0** lorsqu’il a fait ce qu’il devait, avec le code **2** en cas de refus délibéré (un point de contrôle s’est déclenché, une prémisse a échoué, un argument a été refusé) et avec le code **1** en cas de plantage. Un refus ou un plantage affiche exactement une ligne du format `<TOOL>_HALT {json}` dont l’enregistrement contient six clés — `tool`, `outcome`, `gate`, `error`, `message`, `evidence` — où `outcome` est l’une des trois phrases (`HALTED — a gate fired`, `REFUSED — the tool declined to proceed`, `ÉCHEC — une erreur non gérée `), `message` est le texte du refus lui-même, et `evidence.clause` est le mot lisible par machine sur lequel un appelant se base ; le vocabulaire de la clause est conservé par la suite (`tests/test_refusal_clauses.py`). Le succès est un indicateur que l’outil obtient grâce à un effet (`BUILD_PAYLOAD_OK`, `RIG_OK`, `ENCODE_OK`, …), et non jamais uniquement grâce au code de sortie — `blender -b -P` se termine avec le code 0 lorsqu’une exception d’un script se propage, ce qui explique pourquoi les outils côté Blender contiennent la même ligne d’arrêt dans un gestionnaire local et cinq des outils du banc d’essai écrivent également un `halt.json` à côté des résultats qu’ils n’ont pas produits. La seule implémentation du contrat CPython est `armature_core.parts.run_tool_main` ; sa docstring est la spécification. L’enregistrement est affiché sous forme de JSON strict, le texte de l’outil étant conservé tel quel ; lorsque l’encodage du terminal ne peut pas prendre en charge un caractère, la ligne revient à un `\uXXXX` d’échappement pour ce caractère plutôt que d’échouer, et le code de sortie n’est affecté d’aucune manière.

Trois familles de refus sont apparues avec l’étape C (2026-09-06) et se lisent de la même manière partout :

- **Un outil qui écrirait sur les artefacts d’une exécution précédente** — le graphique et le journal d’une construction, les images d’un rendu — refuse avec la clause `output_already_exists` et les nomme (une construction nomme les deux fichiers et les deux hachages) ; `--overwrite` les remplace, et le journal de succès contient alors `out_dir_pre_existed` et `overwrote`. Le seul générateur de dépenses qui se trouve sous le `out_dir_not_empty` de la porte CANON refuse un `--out` non vide un point de contrôle plus tôt et n’accepte aucun indicateur.
- **Un outil qui attend** — un encodage, un décodage, un rendu, un téléchargement — affiche une ligne de progression en minuscules sur stderr, `<tool> <stage> <done>/<total>  elapsed <e>s  bound <b>s`, avant et après l’attente (et par élément lorsque le travail est quantifiable) ; stdout ne contient toujours qu’une seule ligne de succès ou d’arrêt. Chaque sous-processus contient une limite dérivée du travail, et une limite qui est atteinte est un refus par nom (`ffmpeg_exceeded_the_time_bound`, `downloader_exceeded_the_time_bound`) qui nomme le travail partiel sur le disque et ne réessaie jamais — sur le chemin payant, une nouvelle tentative entraîne des dépenses qui n’ont pas de compensation.
- **Un point de contrôle qui se déclenche après l’existence du répertoire de sortie** l’indique : le refus nomme le répertoire, indique qu’il contient un travail partiel et qu’il n’est pas un résultat, et indique la prochaine étape prise en charge.
...
Trois leviers d’environnement, tous facultatifs : `PYTHONPATH=E:/AI/record-index` (la copie de travail sœur que les tests d’index importent ; sans cela, ces tests sont ignorés par nom), `ARMATURE_BLENDER` (l’exécutable Blender que les accessoires qui pilotent Blender exécutent ; sans cela, ou en dehors de ce banc d’essai, ces tests sont clairement ignorés) et `ARMATURE_GIT` (le git que le test d’empaquetage invoque). La recette exacte de CI est le travail `python-tests` dans `.github/workflows/ci.yml` ; `verify.ps1` exécute la même suite deux fois (une fois sous `-O`) puis la construction du package.

Le bloc `ceiling` d’une spécification de graine contient les nombres qui lient — `submissions`, la division par bras ou par vague, `counted_in`, `note`. L’historique des corrections qui les sous-tend se trouve à l’adresse [specs/ceiling-why-machine-readable.md](specs/ceiling-why-machine-readable.md) (déplacé là-bas à partir de huit copies identiques dans la spécification le 2026-09-06 ; `specs/*.json` est passé de 80 321 à 45 585 octets).
...
| Python | `>=3.11,<3.15`, conformément au paquet ; l’intégration continue exécute les versions 3.11 et 3.13, et l’environnement virtuel de test exécute la version 3.14. `pip install armature-studio` installe numpy, opencv-python-headless, Pillow et matplotlib, qui sont les dépendances d’exécution réellement importées par le code (déclarées depuis la version 3 du contrôle sanitaire ; une installation propre était utilisée pour importer `armature_core`, puis cela échouait lors du premier appel de dessin). pytest est la seule dépendance utilisée uniquement pour les tests ; l’intégration continue fixe opencv à la version de l’environnement de test, car les tests de rasterisation de pose vérifient la stabilité des données rasterisées. |

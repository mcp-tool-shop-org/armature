...
|---|---|---|---|---|
| **`wan22-14b-t2v-technically_color.safetensors`** | CivitAI grant matrix (API-fetched) | **YES — credit required** | [API 2106471](https://civitai.com/api/v1/models/2106471), fetched 2026-08-13, creator renderartist | ⚑ **Corrected same day:** the first fetch read the page's "Apache 2.0" badge as the LoRA's licence; the structured API grant is the creator's actual declaration: `allowCommercialUse ["RentCivit","Rent","Image"]` (third-party-service use + image-commercial granted), `allowNoCredit: false` → **published footage from this LoRA credits the creator** — a credits-line obligation riding the disclosure note. Origin files are a tier-labeled HN/LN pair (`Technically_Color_WAN_2_2_T2V_v1_…_high/low_noise`); the Cloud serves one file — tier identity NOT VISIBLE, premise rides the spec |
| ~~`wan22-candid_photography.safetensors`~~ | CivitAI grant matrix (API-fetched) | **NO — corrected from a wrong YES, owned** | [API 1925758](https://civitai.com/api/v1/models/1925758), fetched 2026-08-13 | ⚑ this seat's first fetch took the page's "Apache 2.0" for the LoRA's grant; the API matrix is `allowCommercialUse ["RentCivit"]` **only** — no image-commercial, no third-party-service use. Both rights our route needs are withheld. Killed by the sharper instrument; the wrong row stood for under an hour |
| **`WAN2.2-HighNoise/LowNoise_SmartphoneSnapshotPhotoReality_v3_by-AI_Characters`** (pair) | CivitAI grant matrix (API-fetched) | **YES — REVIVED on the Director's ask, by document** | [API 1834338](https://civitai.com/api/v1/models/1834338), fetched 2026-08-13, creator AI_Characters | `allowCommercialUse ["Image","RentCivit","Rent","Sell"]`, `allowNoCredit: true`, `allowDerivatives: true` — the most permissive grant in the field. The API's v3 file names match the served pair exactly (the Cloud's double-`.safetensors` on HIGH is a provisioning artifact) |
...
| ~~`wan22-14b-t2v-80s_fantasy_movie`~~ | CivitAI grant matrix (API-fetched) | **NO — upgraded from NOT-RETRIEVED to a retrieved kill** | [API 789313](https://civitai.com/api/v1/models/789313), fetched 2026-08-13, creator ArsMachina | `allowCommercialUse ["RentCivit","Image"]` — image-commercial granted, but **"Rent" (third-party generation-service use) is withheld**, and generation through Comfy Cloud is exactly that use; also `allowDerivatives: false`, `allowNoCredit: false`. The Director asked for its revival 2026-08-13; the document itself withholds the needed right — revival would take the creator's grant, not an export. Wan versions are additionally T2I-labeled |

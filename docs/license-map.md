...
| **Wan2.2-Fun-A14B-Control-Camera** | Apache-2.0 | **YES** | [HF card](https://huggingface.co/alibaba-pai/Wan2.2-Fun-A14B-Control-Camera), fetched 2026-08-12 (consult #7 ruling) | camera-controlled video (81f @ 16 fps); serves `WanCameraEmbedding` → `WanCameraImageToVideo` (both core nodes, GPL row). ⚠ this card is **silent on output rights** — the base-family output clause is not on it; Comfy Cloud's ToS ownership row governs our generations service-side. |

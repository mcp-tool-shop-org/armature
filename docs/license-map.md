...
| **Wan 2.6/2.7 partner tier** (`wan2.6-i2v` · `wan2.6-r2v` · `wan2.7-r2v` · `wan2.7-t2v` · `wan2.7-videoedit`) | provider-side terms NOT RETRIEVED, attempted 2026-08-12: `wan.video/terms` 301s to `create.wan.video/terms`, a JS-rendered shell with no text (the DingTalk disease — the Director's browser-export path is the known cure); Alibaba's master international Product ToS contains **no Model Studio / Tongyi / AI-output clauses** (service-specific addendum not located). The **Comfy-side ToS row above is retrieved and stands** ("Customer retains all right, title, and interest in and to… Output") — but this map's standing treatment for partner tiers (Kling, MiniMax, Seedance rows) requires the provider document too | **E13 (the composed-route probe) and any 2.6+ generation** — blocked until the provider document is fetched **OR the Director rules the Comfy ToS row sufficient** (CONDITIONAL — his decision, surfaced contrastively 2026-08-12; the E13 spec encodes both unblock paths) |

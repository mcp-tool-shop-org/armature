...
- [x] `[all]` Version in manifest matches git tag (2026-08-15) — ⚑ **this SKIP re-opened exactly as its own text said it would, the moment a manifest existed.** It previously read "there is no manifest… re-opens the moment one exists"; `pyproject.toml` and `npm/package.json` now both declare `0.2.0`, and the check is mechanical rather than clerical: `.github/workflows/publish.yml`'s `verify` job compares the git tag against both manifests and fails the release before either registry is reached, because a tag that disagrees with its metadata publishes a version nobody asked for

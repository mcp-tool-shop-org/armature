...
- [ ] `[all]` SKIP: version in manifest matches git tag. **There is no manifest** — no `pyproject.toml`, no root `package.json`, nothing that declares a version, because nothing is published. The version lives in the CHANGELOG heading and in this file, and `v0.1.0` is cut against the commit that carries them. Re-opens the moment a manifest exists, and the reason is written this way so that it does
...
- [ ] `[npm]` SKIP: nothing publishes to npm. No root `package.json`, no workspace packages, no tarball surface for `shipcheck pack` to inspect. `site/package.json` is `private` build tooling for the landing page, not a publishable package
- [ ] `[npm]` `[pypi]` SKIP: `engines.node` / `requires-python`. No manifest exists to carry either field. The runtime versions this repo actually runs on are stated in the README's **Running it** table and pinned in CI (`python-version: "3.13"`, `node-version: 22`) — which is where a reader can check them today
- [x] `[npm]` Lockfile committed (2026-08-13) — `site/package-lock.json` is tracked, and both `ci.yml` and `pages.yml` run `npm ci` rather than `npm install`, so the build that runs in CI is the build verified on the rig; a lockfile mismatch is a red job rather than a silent resolve. The `[pypi]` half is skipped: no wheel or sdist is built because nothing is published
...
- [ ] `[all]` Translations (polyglot-mcp, 8 languages) — **outstanding, and deliberately so.** Translations run locally on TranslateGemma and land in the same commit as the English source, **before** the release tag is cut, because a release tag is immutable and stale translations under it are forever. The English README is final as of this treatment; the run is the advisor's next step

...
F-4150910d). PERFORMER and WIRE are census rows with no ratified surfaces file, so
`--no-canon` is the escape that row requires and it announces itself in the record.
BLACKGUARD now carries surfaces; it is no longer an identity-only escape subject.

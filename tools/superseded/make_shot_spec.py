...
HELP_PROG = "python tools/superseded/make_shot_spec.py"
...
def main(argv=None):
    ...
    ap.add_argument("--motion", required=True,
                    help="the authored motion record whose ground truth the camera is "
                         "solved against; read only")
    ap.add_argument("--glb", required=True,
                    help="the performer GLB the spec names, recorded with its digest")
    ap.add_argument("--out", required=True,
                    help="the shot spec JSON to write. Compensator: delete it; owner: the "
                         "executor session")
    ap.add_argument("--name", default="E08-bar-approach",
                    help="the shot's name inside the spec (default E08-bar-approach)")

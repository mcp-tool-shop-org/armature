...
| 测试 | **1183 个通过，针对骨骼进行测试**（13 个跳过，于 2026-08-13 在 E14 关闭合并时衡量），并且低于 `-O`；CI 执行的是一个运行者真正可以执行的内容——本地资产 **明显跳过**。 |
| 状态 | **v0.1.1 于 2026-08-13 发布**——这是记录的当前标记状态，包含 E14 的关闭（v0.1.0 是第一个标记状态，在同一天稍早时被切断）；该记录是文档树，并且已经完成。 |

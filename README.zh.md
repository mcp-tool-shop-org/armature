...
`armature_core` 从 PyPI 安装（见上文）；**实验记录和渲染工具**是此仓库，克隆并运行——没有服务，没有守护进程。每个工具都直接调用：
...
每个 CPython 工具在完成其说明的操作时退出代码为 **0**，在发生明确的拒绝时（触发了门控，前提失败，拒绝了参数）退出代码为 **2**，在发生崩溃时退出代码为 **1**。拒绝或崩溃会打印完全一行，格式为 `<TOOL>_HALT {json}`，其记录包含六个键——`tool`、`outcome`、`gate`、`error`、`message`、`evidence`——其中 `outcome` 是三个句子之一（`HALTED — a gate fired`、`REFUSED — the tool declined to proceed`、`FAILED — an unhandled error`), `message` 是拒绝的文本，并且 `evidence.clause` 是调用者分支所依据的机器可读的单词；子句词汇由测试套件保存（`tests/test_refusal_clauses.py`）。成功是一种工具通过某种效果获得的信号（`BUILD_PAYLOAD_OK`、`RIG_OK`、`ENCODE_OK`、……），而不是单独的退出代码——`blender -b -P` 在脚本的异常传播时退出代码为 0，这就是为什么 Blender 端的工具通过本地处理程序和五个构建工具中的五个来传递相同的停止信号，并且还会写入 `halt.json`，以指示它们未生成的输出。CPython 合约的唯一实现是 `armature_core.parts.run_tool_main`；其文档字符串是规范。记录以严格的 JSON 格式打印，工具自身的散文保留为散文；如果终端的编码无法承载某个字符，则该行将回退到该字符的 `\uXXXX` 转义，而不是停止打印，并且退出代码也不会受到影响。

三个拒绝类别是在阶段 C 通过（2026-09-06）时出现的，并且在所有地方都读取相同的内容：

- **一个工具会覆盖先前运行的工件**——构建的图形和记录、渲染的帧——拒绝时使用子句 `output_already_exists` 并命名它们（构建会命名两个文件和两个摘要）；`--overwrite` 替换它们，然后成功记录会包含 `out_dir_pre_existed` 和 `overwrote`。位于 CANON 门控的 `out_dir_not_empty` 下的唯一支出构建器，会在提前一个门控处拒绝一个非空的 `--out`，并且不接受任何标志。
- **一个工具会等待**——编码、解码、渲染、下载——在等待之前和之后（以及在可以计数的工作项中），将小写进度行打印到标准错误输出，打印 `<tool> <stage> <done>/<total>  elapsed <e>s  bound <b>s`；标准输出仍然只包含一个成功或停止信号。每个子进程都包含一个基于工作量的边界，并且达到该边界会因名称而导致拒绝（`ffmpeg_exceeded_the_time_bound`、`downloader_exceeded_the_time_bound`），该拒绝会命名磁盘上的部分工作，并且不会重试——在付费路径中，重试会消耗没有补偿的积分。
- **一个门控在输出目录存在后触发**，它会说明这一点：拒绝会命名该目录，说明它包含部分工作，并且不是结果，并且说明支持的下一步操作。
...
三个环境控制杆，都是可选的：`PYTHONPATH=E:/AI/record-index`（索引测试导入的同级工作副本；如果没有它，这些测试会因名称而跳过）、`ARMATURE_BLENDER`（驱动 Blender 的测试运行的 Blender 可执行文件；如果没有它，或者不在此构建环境中，这些测试会明显跳过）和 `ARMATURE_GIT`（打包测试调用的 git）。CI 的确切配方是 `python-tests` 任务，位于 `.github/workflows/ci.yml` 中；`verify.ps1` 运行相同的测试套件两次（一次在 `-O` 下），然后运行打包构建。

种子规范的 `ceiling` 块包含绑定数字——`submissions`，每个分支或每个波次的拆分，`counted_in`、`note`。其背后的更正历史记录位于 [specs/ceiling-why-machine-readable.md](specs/ceiling-why-machine-readable.md)（于 2026-09-06 移动到此处，此前有八个相同的规范内副本；`specs/*.json` 从 80,321 字节减少到 45,585 字节）。
...
| Node | `armature` 启动器（`npm/`）在持续集成中分别在 18 和 22 上进行测试；位于 `site/` 的站点在 22 上进行构建。 |

...
| Test | **8051 superato nei test sulla piattaforma** (59 test saltati, misurati il 2026-09-07 dopo la correzione del pin per l'esecuzione della funzionalità — 7538 nella versione v0.4.0, 7811 dopo la fase D), risultati identici sotto `-O` durante la prova che ha registrato lo stato; i test CI valutano ciò che un esecutore può fare in modo onesto: le risorse locali della piattaforma **vengono saltate in modo evidente**. |
| Stato | **v0.5.0** — miglioramenti visivi nella fase D e superamento dei test di una funzionalità aggiuntiva rispetto alla versione v0.4.0. Approvata la procedura di invio all'interno del repository (`build_submit_payload.py`, prova), installati gli strumenti CLI `canon` / `verify` / `spec` / `donor`, 8051 test. Non è la versione 1.0.0: la voce relativa ai miglioramenti è ancora aperta. |

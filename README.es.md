...
Fundada el **10 de agosto de 2026**. Doce experimentos cerrados y la tesis ha pasado de estar *en fase de prueba* a ser **medida a nivel de producto**: el personaje ha bailado en pantalla dos veces, una vez controlado por su propio esqueleto y otra vez libre, y un mundo modelado ahora se mantiene hasta el último fotograma en dos semillas (E12), todo juzgado por la mirada del director. La auditoría del arco fundacional está disponible en
[docs/audit-first-arc.md](docs/audit-first-arc.md); la postura desde el 12 de agosto de 2026 es un repositorio monorepo para aprendizaje: los experimentos demuestran caminos, ninguna ruta es canónica por inercia (CLAUDE.md).

| | |
|---|---|
| Experimentos | **E01–E12 cerrados** (E05 retirado debido a una premisa falsa); el arco de control (E01–E06) · reparación del esqueleto + aprobación del modelo (E07) · **la primera toma renderizada** (E08) · la línea base de cadena limpia (E09) · se adopta un sistema de control más denso (E10) · la ruta sin control, tres fases que conducen a un fallo instructivo (E11) · **la ruta libre obtiene un mundo**, y la línea base de configuración se reduce al catálogo 6.0 / uni_pc (E12) · **E13 lanzado el 13 de agosto de 2026**: la sonda de ruta compuesta, referencias diseñadas en la capa wan2.7 de referencia a vídeo |
| Rutas | **dos, más una en fase de prueba** — la **ruta controlada** (esquema AAPose renderizado → Animate; probado a nivel de toma, reservado para la creación de animaciones con IA) · la **ruta libre** (fotograma inicial diseñado en GLB → capas I2V / cámara en la línea base 6.0 / uni_pc; la identidad se mantiene sin anclar y un mundo modelado se mantiene en dos semillas) · la **ruta compuesta** (referencias diseñadas en una capa de bloqueo de identidad alojada — la sonda E13; su nota de divulgación se incluye en las especificaciones según la ley de divulgación por ruta) |
| Gasto | 22 pruebas en el arco fundacional a 4 créditos cada una; el arco E08–E12 registró **0 créditos** en cada envío (facturación por hora de GPU) según los límites por experimento: E12 gastó 4 de sus 6 envíos permitidos, y el resto no se utilizó. |
...
| Estado | **de nuevo público a partir del 13 de agosto de 2026** (privado por elección del 11 al 13 de agosto): organizando para un lanzamiento de **v0.1.0**; el registro es el árbol de documentos y está completo. |

### Qué se mide (el arco actual)

- **La identidad se mantiene**: controlada (E08: la cara se reconoce como la del gemelo en toda la toma) *y* sin anclar (fase 1 de E11: cada característica hasta el último fotograma, sin referencia, sin visión de recorte, sin señal de control). La mirada del director es el veredicto definitivo en ambos casos.
- **La cámara obedece al control explícito a un píxel** en los pesos de la capa de la cámara (fase 3 de E11) y se desplaza sin que se le ordene (fase 1 de E11).
- **La densidad mueve la señal, no el rendimiento** (E10): el remuestreo suaviza los pasos en un 41 %, el rendimiento en un 8,6 %; adoptado de todos modos a ojo: más fotogramas por segundo se ven mejor.
- **Una fila de licencia no es una reclamación de cableado** (fase 2 de E11): un modelo con licencia Apache y un gráfico que nunca lo cargó produjeron 65 fotogramas de ruido con todas las puertas en verde. Ahora existe la puerta PAIR.
- **La composición de la escena es volátil según la semilla** (E10 / E11): el mismo texto recompuso por completo el mundo entre semillas. **Una reclamación de escena necesita dos semillas antes de que sea una propiedad.**
- **Un mundo modelado se mantiene** (E12): una habitación real en el fotograma inicial sobrevive hasta el último fotograma en dos semillas en la capa de la cámara, con un atributo variable atribuido a la imagen inicial mediante diferencia de campo. La misma capa mantuvo un vacío de previsualización como un vacío (fase 3 de E11): los mundos se diseñan y luego se mantienen.
- **El catálogo 6.0 / uni_pc es la línea base de la capa de la cámara** (E12): la premisa heredada de 3.5 / euler cayó a su propio nivel: con la configuración del catálogo, las mismas semillas que perdieron una cabeza y le hicieron crecer un brazo mantienen la figura hasta el fotograma 80. El coste se indica: una mayor adherencia impuso la **cláusula de identidad sin límites** al grupo en una de las dos semillas; el mensaje con ámbito en el sujeto es la palanca promovida.

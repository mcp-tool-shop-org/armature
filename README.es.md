...
`armature_core.blender_scene` es el único módulo que importa `bpy`; `armature check` lo informa como `needs-blender` en lugar de como un defecto. `check` indica lo que es cierto de la instalación desde la que se ejecutó, no simplemente lo que importa: también resuelve las importaciones locales de la función, por lo que una extracción de código fuente o una instalación de `--no-deps` que falte OpenCV, Pillow o matplotlib informa `needs-cv2` / `needs-PIL` filas, imprime `UNRESOLVED:` y sale con el código 1; antes, en una instalación cuyo código de dibujo no podía ejecutarse, imprimía "todos los módulos resueltos".
...
| Umbrales de gasto | **Gate CANON** rechaza un envío de pago cuyo tema no se puede nombrar en relación con un canon legible por máquina: la superficie es la fila, un ocupante nulo es un **hueco en lugar de una ausencia**, y ambas direcciones se verifican (el mensaje cubre el canon; todo en el mensaje *es* canon). Se activa **antes** de que se cree el directorio de salida, dentro de cada uno de los siete generadores de carga útil, porque el paso irreversible que posee este repositorio es escribir una carga útil. La solución se basa en el censo: `--no-canon` en un tema que *tiene* canon se rechaza, no se acepta; y, dado que pasó la primera verificación de estado, es **muy estricto en cada gasto**: cada generador imprime `[canon] ARMED: <subject>` y, en la solución del censo, `[canon] UNGATED: <subject> — <the census row's reason>`, por lo que un registro de compilación distingue un hueco ratificado de un tema cuyo canon nunca se escribió; y registra el veredicto en `gates.CANON`, por lo que ningún registro puede dejar sin respuesta la pregunta de si el canon estaba activo o se evitó. |
| Pruebas | **7538 aprobados en el sistema de pruebas** (64 omitidos, medidos el 2026-09-06 al final de la ejecución de verificación; 1359 antes, 1781 después de la primera pasada, 7181 después de la etapa B), idénticos bajo `-O`; CI ejecuta lo que un ejecutor puede hacer honestamente: los activos locales del sistema de pruebas **se omiten visiblemente**. |
| Estado | **v0.4.0** — la ejecución de verificación: 832 problemas solucionados en tres etapas, un contrato de interrupción para cada instrumento, cada rechazo que indica lo que midió y un conjunto de pruebas cinco veces más grande. v0.3.0 le dio al registro una puerta de gasto y un índice que se verifica a sí mismo. `armature_core` se envía a PyPI como `armature-studio` y a npm como `@mcptoolshop/armature-studio`, publicado desde una etiqueta por OIDC sin ninguna ficha de larga duración en ningún lugar. |
...
`armature_core` se instala desde PyPI (arriba); el **registro del experimento y los instrumentos de renderizado** son este repositorio, clonado y ejecutado; no hay servicio ni demonio. Cada instrumento se invoca directamente:
...
Cada herramienta, según cómo se ejecuta, con su propia descripción de una línea: [docs/tools.md](docs/tools.md) (generado a partir de las cadenas de documentación, 2026-09-05).

### Leyendo una interrupción

Cada instrumento de CPython sale con **0** cuando hizo lo que dice, **2** en un rechazo deliberado (se activó una puerta, falló una premisa, se rechazó un argumento) y **1** en caso de un error. Un rechazo o un error imprime exactamente una línea del formato `<TOOL>_HALT {json}`, cuyo registro contiene seis claves: `tool`, `outcome`, `gate`, `error`, `message`, `evidence`; donde `outcome` es una de las tres oraciones (`HALTED — a gate fired`, `REFUSED — the tool declined to proceed`, `FALLÓ: un error no controlado`), `mensaje` es el texto del rechazo, y `evidence.clause` es la palabra legible por máquina en la que se basa un llamador; el vocabulario de la cláusula lo mantiene el conjunto de pruebas (`tests/test_refusal_clauses.py`). El éxito es una señal que la herramienta obtiene por un efecto (`BUILD_PAYLOAD_OK`, `RIG_OK`, `ENCODE_OK`, …), nunca solo el código de salida; `blender -b -P` sale con 0 cuando la excepción de un script se propaga, por lo que las herramientas del lado de Blender llevan la misma línea de interrupción a través de un controlador local y cinco de las herramientas del sistema de pruebas también escriben un `halt.json` junto a las salidas que no produjeron. La única implementación del contrato de CPython es `armature_core.parts.run_tool_main`; su cadena de documentación es la especificación. El registro se imprime como JSON estricto, dejando la prosa de la herramienta como prosa; donde la codificación del terminal no puede contener un carácter, la línea recurre a un escape `\uXXXX` para ese carácter en lugar de fallar al imprimir, y el código de salida tampoco se ve afectado.

Tres familias de rechazos llegaron con la etapa C (2026-09-06) y se leen de la misma manera en todas partes:

- **Una herramienta que sobrescribiría los artefactos de una ejecución anterior** — el gráfico y el registro de una compilación, los fotogramas de un renderizado — se rechaza con la cláusula `output_already_exists` y los nombra (una compilación nombra ambos archivos y ambos resúmenes); `--overwrite` los reemplaza y el registro de éxito luego contiene `out_dir_pre_existed` y `overwrote`. El único generador de gasto que se encuentra debajo de la `out_dir_not_empty` de Gate CANON rechaza un `--out` no vacío una puerta antes y no toma ninguna marca.
- **Una herramienta que espera** — una codificación, una decodificación, un renderizado, una descarga — imprime una línea de progreso en minúsculas a stderr, `<tool> <stage> <done>/<total>  elapsed <e>s  bound <b>s`, antes y después de la espera (y por elemento donde el trabajo es contable); stdout sigue conteniendo solo la única línea de éxito o interrupción. Cada subproceso contiene un límite derivado del trabajo, y un límite que se alcanza es un rechazo por nombre (`ffmpeg_exceeded_the_time_bound`, `downloader_exceeded_the_time_bound`) que nombra el trabajo parcial en el disco y nunca vuelve a intentarlo; en la ruta de pago, un reintento gasta créditos que no tienen compensación.
- **Una puerta que se activa después de que existe el directorio de salida** lo indica: el rechazo nombra el directorio, dice que contiene trabajo parcial y que no es un resultado, y dice cuál es el siguiente paso admitido.
...
Tres palancas de entorno, todas opcionales: `PYTHONPATH=E:/AI/record-index` (la copia de trabajo hermana que las pruebas de índice importan; sin ella, esas pruebas se omiten por nombre), `ARMATURE_BLENDER` (el ejecutable de Blender que ejecutan los accesorios de Blender; sin él, o fuera de este sistema de pruebas, esas pruebas se omiten visiblemente) y `ARMATURE_GIT` (el git que invoca la prueba de empaquetado). La receta exacta de CI es el trabajo `python-tests` en `.github/workflows/ci.yml`; `verify.ps1` ejecuta el mismo conjunto de pruebas dos veces (una vez bajo `-O`) y luego la compilación del paquete.

El bloque `ceiling` de una especificación de semilla contiene los números que vinculan: `submissions`, la división por brazo o por ola, `counted_in`, `note`. El historial de corrección detrás de ellos tiene un hogar en [specs/ceiling-why-machine-readable.md](specs/ceiling-why-machine-readable.md) (movido allí desde ocho copias idénticas dentro de la especificación el 2026-09-06; `specs/*.json` pasó de 80.321 a 45.585 bytes).
...
| Python | `>=3.11,<3.15` por paquete; CI ejecuta 3.11 y 3.13, el entorno virtual del sistema de pruebas ejecuta 3.14. `pip install armature-studio` instala numpy, opencv-python-headless, Pillow y matplotlib: las dependencias de tiempo de ejecución que el código realmente importa (declaradas desde la ejecución de verificación de la ola 3; una instalación limpia solía importar `armature_core` y luego fallaba en su primera llamada de dibujo). pytest es la única dependencia solo para pruebas; CI fija opencv a la versión del sistema de pruebas porque las pruebas de rasterización de pose afirman la rasterización estable en bytes. |
...
| Node | el lanzador `armature` (`npm/`) se prueba en las versiones 18 y 22 en el entorno de integración continua; el sitio que se encuentra en `site/` se compila en la versión 22. |

...
| **E07** — the skeleton | **SKELETON APPROVED with reservation · BINDING: HARD FAIL, consult out** (branch E07-run, parked). The arc: bone heat bound zero weights (halt upheld, liveness clause now law); pivots re-derived onto measured ball centers after the Director caught them off at his zoom (elbows 27–28% of segment off); skeleton approved 2026-08-11 with his reservation on the record (*"It's approved, but I'm not really happy with it"*; skeleton v2 = articulated fingers, needs an F-series hand-mesh iteration too). Then **both bindings failed at his eye** — envelope tears, rigid-per-segment steps at every joint — his ruling verbatim: *"This is a hard fail."* The route question is out to the Comfy Agent: **[docs/comfy-consult-5-brief.md](docs/comfy-consult-5-brief.md)** (rigid-parts split / proxy weight-transfer / repair-then-heat / current auto-rig ecosystem, licence axis throughout). E07 stays open; nothing binds until the consult returns and the Director rules a route |

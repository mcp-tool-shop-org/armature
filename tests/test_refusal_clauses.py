...
RECORDED_POPULATION = frozenset({
    ...
    # WAVE 37 pin-fix: ReliftSelfComparison crossed the two-site threshold (measured).
    "ReliftSelfComparison",
...
def test_the_policed_population_is_derived_from_the_tree_and_has_not_grown_silently():
    ...
    # WAVE 37 pin-fix: 133 -> 134 — ReliftSelfComparison (measured).
    assert len(POLICED) == 134, sorted(POLICED)
...
def test_a_class_raised_from_exactly_one_site_is_deliberately_not_policed():
    ...
    # WAVE 37 pin-fix: ReliftSelfComparison left single into POLICED;
    # single 20 -> 19, family_here 18 -> 17.
    assert len(single) == 19, sorted(single)
    family_here = single & _family_classes_defined_under_tools()
    assert len(family_here) == 17, sorted(family_here)
...
def test_the_delegated_edge_sees_the_three_classes_the_literal_walk_could_not():
    ...
    # WAVE 37 pin-fix: MakeSheetError literal 8 -> 10 / total 9 -> 11 (measured).
    assert len(old.get("MakeSheetError", ())) == 10, sorted(old.get("MakeSheetError", ()))
    assert len(RAISE_SITES["MakeSheetError"]) == 11, sorted(RAISE_SITES["MakeSheetError"])
    ...
    # WAVE 37 pin-fix: PosePackError literal 3 -> 9 / total 6 -> 12 (measured).
    assert len(old.get("PosePackError", ())) == 9 and len(RAISE_SITES["PosePackError"]) == 12
...
#: WAVE 37 pin-fix: 911 -> 967 on the merged tree — motion/decimate/camera-path/hdri/
#: relift-self/engine/foot-plant clauses landed with the product; vocabulary re-derived with ==.
RECORDED_CLAUSES = [
    'above_one',
    ...
    'camera_keys_not_for_type',
    'camera_path_key_missing_field',
    'camera_path_key_not_a_dict',
    'camera_path_keys_required',
    ...
    'cloud_history_get_failed',
    'cloud_history_unreadable',
    'cloud_post_failed',
    ...
    'codec_from_missing_recommendation',
    'codec_from_missing_survey',
    'codec_from_not_object',
    'codec_from_unreadable',
    'codec_not_in_survey',
    ...
    'dailies_floor_runs_required',
    'dailies_floor_seeds_required',
    ...
    'decimate_budget_below_two',
    'decimate_budget_not_positive',
    'decimate_requires_budget',
    ...
    'ffmpeg_refused_the_stream_encode',
    'field_absent',
    'field_disagrees',
    ...
    'frames_or_from_motion_required',
    'frames_pair_not_int',
    'frames_pair_shape',
    'frames_requires_densify',
    'frames_without_index',
    'from_motion_excludes_frames',
    'from_motion_fps_drift',
    'from_motion_requires_manifest',
    'from_motion_size_drift',
    'from_motion_stick_size_drift',
    'gait_too_short_for_a_skin_comparison',
    'gate0_fps_mismatch',
    'gate0_meta_not_an_object',
    'gate0_meta_required',
    'gate0_meta_unreadable',
    'gate0_reference_missing',
    ...
    'motion_frame_counts_differ',
    'motion_is_not_a_file',
    'motion_or_retarget_required',
    'motion_pair_incomplete',
    ...
    'pinned_and_fresh_motion_are_the_same_file',
    'pixels_not_a_plane',
    'pixels_unreadable',
    'plan_paths_collide',
    ...
    'trap_codec_requires_allow_trap',
    ...
    'wait_exceeded_the_time_bound',
    'why_not_supplied',
    'world_bounds_without_scene',
    'write',
    'write_camera_path_needs_azimuth_sweep',
    'write_record_needs_record',
    'write_record_not_a_mapping',
    'write_record_unreadable',
    'zero_dimension',
...
SENTENCE_SHAPED_CLAUSES = {
    'orbit radius':
        'render_turnaround.py:934',
    'render visibility':
        'render_turnaround.py:1107',
}

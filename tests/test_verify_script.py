...
def legs_and_their_commands():
    ...
    THE NODE (wave 12, F-7d109c56): the COMMANDS a leg invokes, sliced out of the leg's own
    body, against the flag that skips that leg. The old assertion was `"node" in got.stdout`,
    which is satisfied by the clause that is still there and cannot see the one that went
    missing — and the clause that went missing is npm, not node. Measured 2026-09-04 by
    copying the real `verify.ps1` into a scratch root and running it under pwsh with this
    file's own `STRIPPED_PATH`: `verify.ps1 -NoSite` exits 2 and prints
    `ANDON: not on PATH: node` — npm is not named — while leg 3, the leg `-NoSite` KEEPS,
    runs `npm pack --silent` (:275) and `npm install --prefix $npmroom $tarball` (:288) as
    the npm clean room it mirrors from ci.yml.

    The slice: each `if ($NoX) { ... } else { Invoke-Leg ... }` block, from the guard to the
    next top-level `if ($No`, with comment lines dropped — the steps explain each other, so
    both commands appear in the prose above them and an inventory read off a comment would
    be an inventory read off an explanation.
    """
    ...
    for i, (start, flag) in enumerate(guards):
        ...
        body = "\n".join(line for line in VERIFY[start:end].splitlines()
...
#: Measured 2026-09-04 by running the real script under pwsh with `STRIPPED_PATH`:
#: `verify.ps1 -NoSite` prints `ANDON: not on PATH: node` and does not name npm, while the
#: leg `-NoSite` keeps runs `npm pack --silent` and `npm install --prefix $npmroom`. The
#: `$needed` line (verify.ps1:148-150) is ci-packaging's to correct this wave; a SUBSET
#: assertion, so the entry becomes deletable rather than red the moment it lands.
...
def test_the_halt_names_every_tool_the_selected_legs_shell_out_to(skipped, tmp_path):
    ...
    This is the clause an edit to `$needed` deletes silently. Its predecessor asserted only
    `"node" in got.stdout`, so it was satisfied by the clause that is still there —
    `-NoSite` keeps leg 3, leg 3 runs `npm pack` and `npm install`, and `$needed` asks for
    node alone. On a box with node and no npm, `verify.ps1 -NoSite` passes the pre-flight and
    then discovers npm mid-leg: exactly the failure the block's own comment says it exists to
    prevent ("halt up front rather than discovering it mid-run", :145-147), and the leg's
    recorded outcome then depends on how PowerShell reports the missing command rather than
    on an andon — the same family as the recorded headline defect at verify.ps1:79-81.

    The `$needed` line itself is ci-packaging's to fix (F-7d109c56's other half).
    """
    inventory = legs_and_their_commands()
